...
public class PageFlipView extends GLSurfaceView implements GLSurfaceView.Renderer {
    ...
    public int getCurBackgroundResId () {
        return mPageRender.getCurBackgroundResId();
    }

    /** 获得当前页号 从1开始计 */
    public int getPageNo () {
        return mPageRender.mPageNo;
    }

    /** 刷新绘制页面 */
    public void refresh () {
        mPageRender.refresh();
        mPageRender.refreshFlagFirstPage = true;
        mPageRender.refreshFlagSecondPage = true;
        requestRender();
    }

    /** 跳转页 从第一页计 */
    public void gotoPage (int index) {
        index = Math.max(1, Math.min(index, PageFactory.get().getPagesRealSize()));
        mPageRender.mPageNo = index;
    ...
    public void refreshChapter () {
        mPageRender.mPageNo = 1;
        refresh ();
    }

    /** 仅刷新画笔颜色 */
    public void refreshPaintColor () {
        mPageRender.refreshPaintColor();
        mPageRender.refreshFlagFirstPage = true;
        mPageRender.refreshFlagSecondPage = true;
        requestRender();
    }
